...
class BattelTests2 : AnnotationSpec() {
    ...
    fun `Test Buff of the attacker`() {
        ...
        assertThat(quantomix1BattleStats.getStats().getSpecialAttack()).isEqualTo(250)
    ...
    fun `Test Attack and attacker is dead`() {
        ...
        val exception = assertThrows(DeadQuantomixException::class.java) {
            battle.simpleBattle(
                ...
                attack = attackDeadly, target = quantomix1BattleStats, attackStrategy = damageStrategy()
            )
        }
        assertEquals("The Quantomix which would attack next is already dead!", exception.message)
    ...
    fun `Test with Attack with self damage and self buff and no damage`() {
        ...
        val attackComplex = Attack(
            ...
            specialAttack = false,
            effects = mutableListOf(
                Effects(heal = false, changeStats = Stats(kp = 25, attack = 0, defense = 6, specialAttack = 0, specialDefense = 5, speed = 0), self = true),
        ...
        assertThat(quantomix1BattleStats.getStats().getSpecialAttack()).isEqualTo(300)
        assertThat(quantomix1BattleStats.getStats().getDefense()).isEqualTo(400)
