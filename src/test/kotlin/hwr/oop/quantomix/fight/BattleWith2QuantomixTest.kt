...
class BattleWith2QuantomixTest : AnnotationSpec() {
  ...
  fun performRound(
    ...
    val activeTeamCoach2 = coach2.getActiveQuantomixes(
      numberOfQuantomixInRound = 2,
      battle.quantomixAndBattleStatsMap
    )
    val activeTeamCoach1 = coach1.getActiveQuantomixes(
      numberOfQuantomixInRound = 2,
      battle.quantomixAndBattleStatsMap
    )
  ...
  fun `choosing a target from same team throws exception`() {
    ...
    assertEquals(
      "The chosen target does not belong to the other trainer.",
      exception.message
    )
  }
